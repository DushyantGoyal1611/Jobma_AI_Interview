...
def schedule_interview(role:str|dict, resume_path:str, question_limit:int, sender_email:str) -> str:
    ...
    with engine.begin() as conn:      # Ensures transactional safety: commits on success, rolls back on error.
        ...
        if result:
            ...
            insert_candidate = text("""
                INSERT INTO AI_INTERVIEW_PLATFORM.candidates (name, email, experience, skills, education, resume_path, phone, created_at)
                VALUES (:name, :email, :experience, :skills, :education, :resume_path, :phone, :created_at)
            """)
            ...
            insert_invite = text("""
                INSERT INTO AI_INTERVIEW_PLATFORM.interview_invitation 
                    (candidate_id, role, question_limit, sender_email, status, interview_scheduling_time, created_at)
                VALUES 
                    (:candidate_id, :role, :question_limit, :sender_email, :status, :interview_scheduling_time, :created_at)
                """)
    ...
    return f"Interview scheduled successfully and saved to {name} ({email})."
